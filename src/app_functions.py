...
class Settings:
    def __init__(self):
        self.settings = QSettings("KANGAL Apps", "KANGAL PUBG Mobile Tool")
...
class Optimizer(Registry):
    ...
    def optimize_for_nvidia(self):
        ...
        try:
            nvidia_profile_path = self.resource_path("assets/KANGAL.nip")
    ...
    def ipad_layout_settings(self, reset=False):
        ...
        backup_file = os.path.join(keymap_folder, 'TVM_100.xml.KANGALbackup')
...
class Game(Optimizer):
    ...
    def gen_game_icon(self, game_name):
        ...
        shortcut.Description = "By Mohamed Kamal (KANGALvip) - Discord: KANGALvip"
    ...
    def check_adb_connection(self, first_check=True):
        try:
            ...
            self.adb.sync.pull("/default.prop", self.resource_path(r'assets\testADB.KANGALvip'))
    ...
    def get_graphics_file(self, package: str):
        ...
        local_file_path = self.resource_path('assets/old.KANGALvip')
    ...
    def save_graphics_file(self):
        file_path = self.resource_path("assets/new.KANGALvip")
    ...
    def get_shadow(self):
        ...
        self.adb.sync.pull(user_custom_ini_path, self.resource_path(r'assets\user.KANGALvip'))

        with open(self.resource_path(r"assets\user.KANGALvip")) as file:
            for line in file:
                line = line.strip()
    ...
    def set_shadow(self, value):
        ...
        lines = []
        with open(self.resource_path(r"assets\user.KANGALvip"), "r") as file:
            for line in file:
        ...
        with open(self.resource_path(r"assets\user.KANGALvip"), "w") as file:
    ...
    def push_active_shadow_file(self):
        ...
        files = [
            (self.resource_path(r"assets\new.KANGALvip"), f"{data_dir}/SaveGames/Active.sav"),
            (self.resource_path(r"assets\user.KANGALvip"), f"{data_dir}/Config/Android/UserCustom.ini")
        ]
    ...
    def kr_fullhd(self):
        ...
        safe_path = "/sdcard/KANGAL_safe_folder"
        data_path_for_account = f"/data/data/{self.pubg_package}"

        self.adb.push(self.resource_path('assets\KANGAL_kr.ini'), user_custom_ini_path)

        self.adb.shell(f"KANGALdir -p {safe_path}")
